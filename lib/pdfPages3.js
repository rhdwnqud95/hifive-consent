...
export function drawPage10(c, { form = {}, dateStr = '', studentSigImg, parentSigImg } = {}) {
  // ================= PAGE 10 : 위트젠 관련 조항 + 동의/서명 (단일 페이지) =================
  c.newPage();
  c.title('위트젠 출결관리 서비스 관련 조항', { size: 14 });
  c.space(-6);

  c.subTitle('제1조 (개인정보의 처리 목적)', { size: 10.5 });
  c.space(-3);
  c.paragraph(
    '① 학원관리프로그램의 운영 ② 학생 출석, 귀가, 결석, 수납안내, 공지 메시지 전송 ③ 홈페이지 콘텐츠 제공 ④ 회원제 서비스에 따른 본인확인, 개인식별, 고지사항 전달 ⑤ 회원과의 커뮤니티 구성(학습지도, Q&A 등)',
    { size: 8.8, lineHeight: 12 }
  );
  c.space(4);

  c.subTitle('제2조 (개인정보의 처리 및 보유기간)', { size: 10.5 });
  c.space(-3);
  c.paragraph(
  ...
  c.subTitle('제3조 (개인정보의 제3자 제공)', { size: 10.5 });
  c.space(-3);
  c.paragraph(
  ...
  c.subTitle('제4조 (개인정보처리의 위탁)', { size: 10.5 });
  c.space(-3);
  c.paragraph('① 학원은 "학원관리프로그램"의 운영을 위하여 제1조의 목적으로 위트젠에 개인정보처리를 위탁합니다.', { size: 8.8, lineHeight: 12 });
  c.paragraph('② 위탁하는 개인정보 항목', { size: 8.8, lineHeight: 12 });
  c.paragraph('1) 필수항목 : 학생이름, 학부모휴대번호', { size: 8.8, lineHeight: 12, x: MARGIN + 14, maxWidth: CONTENT_W - 14 });
  c.paragraph('2) 선택항목 : 학교이름, 학년, 반, 진학희망학교, 생일(음/양력), 집전화번호, 학생휴대번호, 학부모이름, 주소, 학생사진, 학생지문정보', { size: 8.8, lineHeight: 12, x: MARGIN + 14, maxWidth: CONTENT_W - 14 });
  c.paragraph('③ 개인정보의 보유 및 이용기간 : 제2조(개인정보의 처리 및 보유기간)과 동일', { size: 8.8, lineHeight: 12 });
  c.paragraph('④ 위트젠은 학원이 위탁한 정보주체의 개인정보에 대해 재위탁 하지 않으며 『개인정보보호법』에 따라 아래의 안전성을 확보 조치를 취하고 있습니다.', { size: 8.8, lineHeight: 12 });
  c.space(1);
  c.bullets([
  ...
  ], { size: 8.3, lineHeight: 11, gap: 1 });

  c.space(4);
  c.subTitle('제5조 (개인정보의 파기)', { size: 10.5 });
  c.space(-3);
  c.paragraph(
  ...
  c.space(8);
  c.hr();
  c.paragraph(`본 문서는 ${c.program.academyName} 신입생 서류의 일부로, 온라인 동의 및 전자서명을 통해 제출되었습니다. 제출 이후 내용의 수정 및 삭제는 불가하며, 원본은 학원에서 보관합니다.`, { size: 8.5, color: GRAY_TEXT });

  // ---------- 위트젠 조항 동의 및 서명 (같은 페이지에서 이어짐) ----------
  c.space(10);
  c.hr();
  c.title('위트젠 출결관리 서비스 관련 조항 - 동의', { size: 12.5 });
  c.space(-6);
  c.checkbox(MARGIN, c.y - 2, !!form.witzenTermsAgree);
  c.paragraph('위트젠 출결관리 서비스 관련 조항의 모든 내용을 확인하였으며 이에 동의합니다.', { size: 9.3, x: MARGIN + 14, maxWidth: CONTENT_W - 14 });
  c.space(6);
  c.field('날짜', dateStr);
  c.space(6);
