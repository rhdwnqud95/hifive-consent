...
function drawPlainChrome(page, font, { smallLogoImg } = {}) {
  const y = 26;
  if (smallLogoImg) {
    const scale = 16 / smallLogoImg.height;
    const lw = smallLogoImg.width * scale;
    const lh = smallLogoImg.height * scale;
    // the logo asset has an opaque light-gray background baked in (matches the
    // GRAY_BAR header elsewhere); on a plain white page it needs a matching
    // backing rectangle behind it, otherwise it shows as a mismatched gray box.
    const pad = 4;
...
export class Cursor {
  ...
  newPage({ chrome = true } = {}) {
    ...
    if (chrome === true) {
      ...
      drawPlainChrome(this.page, this.font, { smallLogoImg: this.assets.smallLogo });
    }
    this.y = PAGE_H - 56;
    return this.page;
  }
