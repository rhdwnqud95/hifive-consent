...
function drawPlainChrome(page, font, { smallLogoImg } = {}) {
  ...
  if (smallLogoImg) {
    const scale = 16 / smallLogoImg.height;
    page.drawImage(smallLogoImg, { x: MARGIN, y: y - 3, width: smallLogoImg.width * scale, height: smallLogoImg.height * scale });
  }
  const text = 'A  서울시 관악구 남부순환로 1791 서원빌딩 4층      T  02-877-0717';
